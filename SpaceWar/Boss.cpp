 ...
 * boss静态单例
 */
 Boss* Boss::createBoss(const char *fileName, int _type)
{
    Boss* boss = new Boss();
 ...
 * boss初始化
 */
void Boss::bossInit(const char* fileName,int _type)
{
    //确定boss类型
    type=_type;
    if(_type==0)
    ...
    //播放动画
    if (type==1)
        this->runAction(Boss::createAnimate2(fileName));
    if (type==2)
        this->runAction(Boss::createAnimate3(fileName));
 ...
 * 制作boss动画
 */
CCAnimate* Boss::createAnimate2(const char* fileName)
{
